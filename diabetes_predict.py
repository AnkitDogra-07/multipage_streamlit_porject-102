...
def app(diabetes_df):
    ...
    bmi      = st.slider("Body Mass Index", float(diabetes_df['BMI'].min()), float(diabetes_df['BMI'].max()))
    ...
    if predictor == 'Decision Tree Classifier':
        if st.button("Predict"):            
            prediction_1, score_1 = d_tree_pred(diabetes_df, glucose, bp, insulin, bmi, pedigree, age)
            ...
            st.write("The accuracy score of this model is", score_1, "%")


    elif predictor == 'GridSearchCV Best Tree Classifier':
        if st.button("Predict"):
            prediction_2, score_2 = grid_tree_pred(diabetes_df, glucose, bp, insulin, bmi, pedigree, age)
            st.subheader("Optimised Decision Tree Prediction results:")
            if prediction_2 == 1:
                st.info("The person either has diabetes or prone to get diabetes")
            else:
                st.info("The person is free from diabetes")
            st.write("The best score of this model is", score_2, "%")